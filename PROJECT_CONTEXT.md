...
- Economic Intelligence (Economy tab): Federal Reserve (FRED) and U.S. Census (ACS 5-year) data integrated into the platform for infrastructure planning context. Top-level **Economy** tab between Map and AI News with four sections — National Economic Pulse (7 KPI indicators), National Trends (SVG time-series charts across Rates & Credit / Inflation & Growth / Labor & Demand / Housing & Construction / Energy & Power Costs, with 1Y–Max ranges), Regional Economic Explorer (county/state choropleth with metric selector across 13 ACS metrics — population, households, income, labor force participation, unemployment, education, home value, rent, homeownership, housing vacancy, broadband, commute time, poverty rate — quantile breaks, and a per-region profile panel that also surfaces supplementary fields where available: county-level Building Permits Survey data reached via FRED's per-county series, county-level BLS QCEW average weekly wage (needs no API key at all), and state-level EIA industrial electricity price, each explicitly distinct from the ACS metrics beside it — plus, for counties, a Data Center Readiness Score: a 0-100 composite computed entirely client-side from nine of the above factors expressed as national percentiles, weighted and combined with graceful weight redistribution when a factor is missing, shown with its full factor breakdown and deliberately excluding the separate zoning/regulatory restriction level), and Infrastructure-Relevant Signals (deterministic rule-based statements, each traceable to a displayed metric). Integrated across the platform rather than isolated: an **Economic Data** layer group in the Map tab's Layers panel (six mutually-exclusive choropleths that never permanently overwrite restriction styles), an Economy section in county detail and Jurisdiction pages, an exploratory Economic Context section in Analytics, and a restrained four-indicator pulse on Home. All API calls happen in Python during GitHub Actions — no API key is ever client-side. See `js/economy.js`, `js/economy-view.js`, `js/economy-map.js`, `css/economy.css`, `data/update_economic_data.py`, `data/economy/`, `.github/workflows/update_economic_data.yml`. Requires the free `FRED_API_KEY` and `CENSUS_API_KEY` repository secrets — both required (Census allowed keyless requests until it began requiring a key for every Data API request on May 12, 2026) — plus an optional `EIA_API_KEY` for the electricity price module (BLS QCEW needs no key). All are free registrations at separate agencies and none can be billed. Until the workflow first runs, every economic panel shows an explicit "not yet measured" state rather than zeros.
- Zoning Intelligence Layer (NoVA milestone): district-level zoning data for three Northern Virginia counties — Loudoun (FIPS 51107, world's largest data center market), Prince William (51153), and Fairfax (51059) — all three with live, GIS-verified district polygon geometry. Complete data architecture with 4 JSON schemas per jurisdiction, full Python pipeline (fetch → normalize → validate → export), weekly GitHub Actions update workflow, and frontend integration (window.ZONING, window.ZONING_MAP, zoning-details.js panel with 5 tabs). Real data-center permitted-use research: 28 of Prince William's 31 real district codes and 3 of Loudoun's 58 (both ordinance-text-verified, high/moderate confidence), plus 10 of Fairfax's 44 (secondary-source-corroborated against Fairfax's September 2024 data-center zoning ordinance amendment, moderate confidence — see `docs/ZONING_PILOT_STATUS.md` for the full by-right/Special-Exception/prohibited breakdown per district). None of the three counties' parcel services publish a native zoning code, so `js/parcel/zoning-geometry.js` resolves a clicked parcel's district via a point-in-polygon spatial join against this geometry, feeding `js/parcel/feasibility.js`'s DC-eligibility scoring. Zoning Districts layer in the Layers panel; clicking a covered county with the layer active opens a zoning intelligence panel showing DC eligibility by district, dimensional standards, permitted uses, overlays, and sources. All data labeled with confidence levels; required legal disclaimer on all output. See `docs/ZONING_ARCHITECTURE.md`, `docs/ZONING_PILOT_STATUS.md`, `data/zoning/`, `js/zoning*.js`, `js/parcel/zoning-geometry.js`, `js/parcel/feasibility.js`, `css/zoning.css`.
- Parcel Site Intelligence schema (`js/parcel/site-intelligence.js`, `window.PARCEL_SITE_INTELLIGENCE`): synthesizes parcel, zoning feasibility, infrastructure proximity, environmental/development constraints, valuations, transactions, and market/policy context for one parcel into a single stable, JSON-serializable object (`build()`), plus a narrower underwriting handoff (`toUnderwritingInputs()`) that deliberately keeps observed facts and required assumptions in disjoint blocks so an assessed value can never silently become an acquisition price. Deterministic, rule-based `findings` (advantages/constraints/unknowns, every statement traced to a named upstream field — never an LLM summary) and `site_status` (`potentially_viable` / `conditional` / `material_constraints` / `insufficient_data`, never "approved" or "buildable") round out the schema. Wired into the parcel panel's Intelligence tab (`js/parcel/panel.js`'s `_renderSiteStatus()`) above the existing suitability/proximity/constraints/sales sections. See `tests/test_parcel_site_intelligence.mjs`, `tests/test_parcel_panel_intelligence.mjs`.
- Parcel source provenance (`js/parcel/provenance.js`, `window.PARCEL_PROVENANCE`): a per-field citation layer (source id/label, confidence tier, source attribute name, fetched-at) that existed but was previously exercised only by test fixtures and by `js/parcel/site-intelligence.js` -- no real production parcel ever carried a `_provenance` record. `js/parcel/connector-arcgis.js`'s `_normalize()` now attaches a `direct-official` record for every canonical field a jurisdiction's `registry.js` `fieldMap` actually verifies against the live ArcGIS service (all 59 registered jurisdictions use this one connector type), so `site-intelligence.js`'s `source_confidence` roll-up reflects real data instead of always reading `unknown`. Surfaced in the parcel panel's Details and Valuation tabs as a small "source" badge (`js/parcel/panel.js`'s `_provenanceBadge()`) with the full citation in a tooltip. Comparable-parcel suggestions (`js/parcel/comparables.js`, `window.PARCEL_COMPARABLES`) were already wired into the panel's Compare tab tray. The project-wide data health dashboard (`data/generate_data_health.py`, `data/data_health.json`, `docs/DATA_HEALTH.md`) now also tracks the parcel registry itself as a `parcels_registry` pipeline, aggregating `data/parcel_health_history.json` (`data/check_parcel_services.mjs --record-history`, scheduled monthly + manual dispatch) with the same confirmed-vs-transient outage logic that script uses internally.
