# Data Health Dashboard

Generated by `data/generate_data_health.py` -- do not hand-edit, run the generator instead.

Project-wide data health: is each pipeline's most recent automated run actually healthy right now, distinct from data_catalog.json's coverage/wiring question. NOT_YET_TRACKED is an honest 'we don't know', never silently reported as OK.

## Pipeline health

| Pipeline | Health | Detail |
|---|---|---|
| policy_pipeline_sources | SOURCE_DOWN | 13 down / 1 transient / 109 total |
| county_page_citations | VALIDATION_FAILURE | 956/1994 unreachable (47.9%) |
| map_data_citations | VALIDATION_FAILURE | 1378/2949 unreachable (46.7%) |
| static_parcel_ingestion | NOT_YET_TRACKED | zero sources registered in sources.json -- pipeline is built and tested but has nothing to run yet (see docs/INFRASTRUCTURE_ASSET_SCHEMA.md-style honesty note in static_ingestion/models.py) |

## Datasets with no automated health signal yet

28 of 28 datasets in data/catalog/dataset_registry.json have no per-dataset automated health check (they are hand-curated JSON or covered only indirectly by the pipeline signals above, not fetched/validated per dataset). Listed here, not defaulted to OK:

- ai_news
- ca_middle_mile_corridor
- data_centers
- economic_census
- facility_sources
- fcc_broadband_fiber_pct
- fema_flood
- fiber_network
- fred_data
- interconnection_queues
- iso_rto
- nwi_wetlands
- pad_us_protected_lands
- parcel_assessment_sales_ownership
- parcel_source_catalog
- parcels_registry
- political_risk
- power_plants
- rail
- restrictions_raw
- roads
- substations
- transmission_lines
- utility_territories
- wastewater
- water_stress
- water_systems
- zoning_jurisdictions

