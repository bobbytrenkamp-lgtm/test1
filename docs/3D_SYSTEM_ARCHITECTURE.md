# 3D Terrain View — Architecture Reference (Phase A + B + C + D + E)

This document covers **Phase A** (integrated 2D/3D terrain foundation), **Phase B** (object system: building volumes, selection, transform gizmo, undo/redo, live site metrics), **Phase C** (parking/road/fence object types, construction phasing, real-parcel-boundary containment checking, object-overlap conflict detection), **Phase D** (generic development templates, the data-center campus generator, saved viewpoints, sun position and shadows), and **Phase E** (image/data export, report integration, presentation mode, accessibility, performance quality tiers, tablet responsive polish) of the 3D site-design/digital-twin system. Two Phase D items — preliminary grading and alternatives/scenario comparison — and one Phase E item — instanced-mesh geometry batching — were evaluated and explicitly deferred rather than shipped shallow; see those sections below.
...
## Phase E: export, report integration, presentation mode, accessibility, performance

**Image/data export** (`js/3d/engine.js`'s `captureSnapshot()`/`getReportData()`, exposed on `window.SCENE3D`, consumed by three new buttons in `#scene3d-export-row` wired in `js/map.js`):
- `captureSnapshot()` does a synchronous `renderer.render(scene, camera)` immediately before `renderer.domElement.toDataURL('image/png')` — `preserveDrawingBuffer` defaults to `false`, so without a forced render-then-read the animate loop's last frame is not guaranteed to still be in the buffer at click time. Returns `null` (not a broken image) if the engine isn't active.
- `exportScene3dImage()` downloads that PNG as `3d-view-<date>.png`.
- `exportScene3dObjectsCSV()` and `exportScene3dJSON()` export the object list / full scene state, following the **exact same** existing patterns already used by `exportScreenerCSV`/`exportWorkspacesJSON` in `js/map.js` — quoted CSV with `""`-escaping, UTF-8 BOM, `Blob` + `URL.createObjectURL` + a temporary `<a download>` + `setTimeout(() => revoke, 1000)`. No new export mechanism was invented.

**Report integration** (`js/parcel/report.js`, the existing per-parcel due-diligence report — extended, not duplicated): `window.SCENE3D.getReportData()` returns `{imageDataUrl, metrics, objects}` (or `null` when zero objects exist), and `js/parcel/panel.js`'s `_openReport()` now passes it through to `PARCEL_REPORT.open(feature, jurisdictionId, scene3d)`. `scene3dSectionHtml(scene3d)` renders a new "Conceptual 3D Site Plan" section — image, metric chips, and a per-object table — **only** when `scene3d` is truthy and has at least one object; a `scene3d` of `null`/`undefined`/`{objects:[]}` produces an empty string, so a report generated by a caller that never touched 3D mode is byte-identical to the pre-Phase-E report (verified in `tests/scene3d.test.js`'s `SCENE3D_REPORT` group). The per-object "Boundary/Overlap Check" column reuses the exact same three-value status vocabulary as the in-scene panel (`Conflict` / `Review setbacks` / `No parcel selected`) and the section repeats the full honesty-boundary disclaimer — it never renders "Approved"/"Compliant"/"Pass"/"Buildable" as a status word, which is directly asserted in the test suite.

**Presentation mode** (`js/map.js`'s `toggleScene3dPresentationMode()`, CSS in `css/scene3d.css`): adds `.scene3d-presenting` to `#scene3d-panel`, which expands the panel to `position:fixed; inset:20px` and hides every editing control (toolbar, templates, campus generator, phase row, object list, setbacks, metrics, viewpoints, sun row, controls, export row, disclaimer) via CSS, leaving only the canvas and a new overlay bar (`#scene3d-present-bar`: Prev / current-viewpoint-name / Next / Exit) for stepping through saved viewpoints. The bar lives in a new `#scene3d-viewport-wrap` sibling wrapper around `#scene3d-canvas-host` rather than inside it — `engine.js`'s `_buildScene()`/`_teardownScene()` do `host.innerHTML = ''` on the canvas host itself, which would silently delete the bar on every re-render if it were a child of that div instead of a sibling.

**Accessibility**: the canvas gets `tabIndex=0`, `role="img"`, and a dynamic `aria-label` (`_updateCanvasAriaLabel()` in `engine.js`, refreshed on every object mutation via `_recomputeConstraints()`) describing live object counts, so a screen-reader user gets an up-to-date textual summary of an otherwise purely-visual canvas. Keyboard camera controls (`_onCanvasKeyDown`): arrow keys orbit (±5°, Shift for ±15°), Up/Down also tilt (clamped 5°–89° polar angle), `+`/`-` zoom (clamped to the same min/max distance as the mouse wheel), `Home` reframes the scene — giving keyboard-only users parity with mouse-drag orbit/zoom, which OrbitControls does not provide on its own. `js/3d/camera.js` checks `prefers-reduced-motion` and disables OrbitControls' inertial damping when set, so camera movement stops the instant input stops rather than continuing to drift. `.scene3d-canvas:focus-visible` gets a visible outline, matching the app's existing `.gis-btn:focus-visible` convention. Object-list rows and their delete buttons already carried proper `aria-label`s from earlier phases (confirmed by inspection, not newly added this phase). **Deliberately not added:** focus-trapping when the panel opens/closes — a grep across every existing floating panel in this codebase (`workspace-panel`, `compare-panel`, `bookmarks-panel`, `screener-panel`) found none of them do this either, so adding it only to the 3D panel would be an isolated inconsistency rather than a genuine improvement; this is a standing cross-panel gap, not one introduced by Phase E, and is listed under "explicitly out of scope" below rather than silently skipped.

**Performance quality tiers**: `js/3d/fallback.js`'s existing `detectWebGL()` low-power heuristic (SwiftShader/llvmpipe software-renderer detection) is now threaded all the way through — `js/3d/index.js`'s `activate()` passes `lowPower: !!cap.lowPower` into `engine.js`'s `activate(opts)`, which stores it as module state and uses it in `_buildScene()` to disable antialiasing, cap the device pixel ratio at 1, disable shadow mapping entirely (`renderer.shadowMap.enabled`/`sunLight.castShadow` both `false`), and halve the shadow map resolution (512px vs. 1024px) when shadows are enabled. A device already flagged as low-power by the existing fallback system now gets a genuinely lighter scene instead of just a warning banner.

**Tablet/touch responsive polish**: `css/style.css` has no existing `pointer: coarse` / `hover: none` media-query convention anywhere in the codebase (confirmed by grep before writing this), so rather than inventing an isolated touch-target pattern for only the 3D panel, the existing `@media (max-width: 700px)` breakpoint in `css/scene3d.css` was extended: larger padding/font-size on every toolbar/template/campus/export/present-bar button and the phase/template `<select>`s, the campus-generator field grid collapses to a single column, viewpoint list rows and their delete buttons get taller tap targets, and the export-row buttons stretch to fill the available width instead of wrapping awkwardly.

**One item evaluated and explicitly deferred, not shipped shallow:**
- **THREE.InstancedMesh geometry batching.** Considered for rendering many similar objects (e.g. a campus generator's grid of data halls) more cheaply. Not implemented: the existing per-object-mesh architecture is load-bearing for `TransformControls` attach-to-select and per-object raycasting hit-testing (both from Phase B), neither of which cleanly supports selecting/dragging an individual instance out of an instanced batch without a substantially more complex custom picking layer. Object counts in this application are realistically in the tens, not the thousands, per scene — nowhere near the range where instancing's win outweighs that architectural risk. Deferred rather than built speculatively for a scale this app doesn't reach.

---

## Explicitly out of scope this pass

Preliminary grading and alternatives/scenario comparison (evaluated and explicitly deferred, see the Phase D section), instanced-mesh geometry batching (evaluated and explicitly deferred, see the Phase E section above), environmental context overlays in the 3D view (evaluated and deferred, see the Phase C section), focus-trapping on panel open/close (a pre-existing gap across every floating panel in this app, not introduced by Phase E — see above), persistent (IndexedDB) tile/object caching, fly/walk-through navigation, keyboard shortcuts for the object editor beyond camera control (undo/redo/delete remain toolbar-button-only, to avoid colliding with existing global shortcuts), non-rectangular footprint shapes, curved or multi-vertex roads/paths, true setback-inset-polygon geometry (see the Phase C constraint-checking honesty boundary), objects following terrain contour/grade (they sit at a nominal flat elevation regardless of terrain undulation), viewpoint renaming (save/jump/delete only), 3D-model export (glTF/OBJ — only image/CSV/JSON), and reconciling the pre-existing drawing-tool (map-level vs. parcel-level) and compare-tool fragmentation identified during the repository audit — a standing cross-cutting cleanup not tied to any single phase.

---

## Manual verification checklist

Automated coverage (`tests/scene3d.test.js`, wired into `tests/run_all.sh`) covers tile math, Terrarium decode, cache/de-dupe/eviction behavior, `scene3d` schema v1→v3 migration, the object store (create/update/remove/restore/metrics, including Phase C's type/phase additions, Phase D's substation type, and backward compatibility at every schema step), the undo/redo command stack, selection events, distance/area math, the constraints module (point-in-polygon, rotated-rectangle SAT overlap, and the "never returns pass/compliant" status vocabulary), solar position (verified against known solstice/equinox reference values), development templates, the campus generator (boundary containment, non-overlap, edge cases for too-small parcels and disabled options), and Phase E's report-integration section builder (`scene3dSectionHtml` — null/empty-objects produce no section for full backward compatibility, a populated scene renders the image/metrics/table/disclaimer, and the status column never renders an Approved/Compliant/Pass/Buildable verdict) — all DOM/WebGL-free pure logic (176 assertions total); the full suite (`tests/run_all.sh`) passes with these additions and no regressions. The following need a real browser and were **not** verified during this implementation pass (Phase A, B, C, D, or E) — the session had no working Chromium/Chrome binary available (a broken symlink in the expected Playwright cache, and `playwright install` is off-limits per environment policy) — so they remain open items for the first real-browser pass on this feature, not confirmed-working claims:
...
- [ ] With objects placed, click "Image (PNG)" in the export row — a real PNG downloads and visually matches the current camera view (not a blank/black frame).
- [ ] Click "Objects (CSV)" / "Scene (JSON)" — both download and open correctly in a spreadsheet app / JSON viewer with the expected fields.
- [ ] With no objects placed, click "Image (PNG)" — a clear "open 3D mode first" style message appears instead of a blank download.
- [ ] Place a few objects, open the parcel report from the parcel panel — a "Conceptual 3D Site Plan" section appears with the captured image, metric chips, and an object table whose status column reads Conflict/Review setbacks/No parcel selected, never a pass/approval claim.
- [ ] Open a parcel report without ever having opened 3D mode — the report renders exactly as it did before Phase E, with no 3D section and no error.
- [ ] Click "Present" — all editing chrome hides, the canvas fills the panel, and the Prev/Next/Exit bar appears; Prev/Next step through saved viewpoints and update the camera; Exit restores the normal editing panel.
- [ ] Close the 3D panel while presentation mode is active, then reopen it — it does not silently stay in presentation mode.
- [ ] Tab to the 3D canvas — a visible focus outline appears; arrow keys orbit the camera, Shift+arrow orbits faster, `+`/`-` zoom in/out, `Home` reframes the scene — all without a mouse.
- [ ] Turn on "reduce motion" at the OS level, then drag-orbit the camera — it stops immediately when the drag ends instead of continuing to glide.
- [ ] With a screen reader active, tab to the canvas — the announced label describes the current object counts (e.g. "2 buildings, 1 parking area"), not a generic "canvas" or empty label.
- [ ] On a device/browser flagged low-power by the existing WebGL capability probe, open 3D mode — shadows are absent (or a visibly lower-resolution shadow map on borderline devices), edges are not antialiased, and the scene still renders and remains interactive rather than stuttering.
- [ ] On a narrow (≤700px) viewport, open the 3D panel and exercise the toolbar/campus-generator/viewpoints/export buttons on a real touchscreen — targets feel comfortably tappable, not cramped to the pre-Phase-E sizing.
